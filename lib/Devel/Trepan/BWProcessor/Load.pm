...
# Part of Devel::Trepan::BWProcessor that loads up debugger commands from
...
# Devel::Trepan::BWProcessor->new in BWProcessor.pm
...
# Instantiate a Trepan::Command and extract info: the NAME
...
sub setup_command($$) 
{
    ...
    my $cmd_obj;
    my $cmd_name = lc $name;
    my $new_cmd = "\$cmd_obj=Devel::Trepan::BWProcessor::Command::${name}" .
        "->new(\$self, \$cmd_name); 1";
    if (eval $new_cmd) {
...
unless (caller) {
    ...
    my $cmdproc = Devel::Trepan::BWProcessor->new;
...
#    $cmdproc->run_cmd({name =>'status'});  # Invalid - not an Array
